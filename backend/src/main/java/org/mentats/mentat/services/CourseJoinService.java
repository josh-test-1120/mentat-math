...
import org.mentats.mentat.repositories.CourseJoinRepository;
...
public class CourseJoinService {
     ...
     * @param courseJoinRequest Course creation request
     * @return Created course
     */
    @Transactional
    public CourseJoin createCourse(CourseJoinRequest courseJoinRequest) {
        System.out.println("CourseName in createCourse: " + courseJoinRequest.getCourseName());
        System.out.println("Quarter in createCourse: " + courseJoinRequest.getCourseQuarter());
        System.out.println("SectionNumber in createCourse: " + courseJoinRequest.getCourseSection());
        System.out.println("Year in createCourse: " + courseJoinRequest.getCourseYear());
        System.out.println("UserID in createCourse: " + courseJoinRequest.getUserId());

        CourseJoin course = new CourseJoin(
                courseJoinRequest.getCourseName(),
                courseJoinRequest.getUserId(),
                courseJoinRequest.getCourseQuarter(),
                courseJoinRequest.getCourseSection(),
                courseJoinRequest.getCourseYear()
        );
        
        System.out.println("Creating course: " + course.getCourseName());
        return courseJoinRepository.save(course);
    }
    ...
    public List<CourseJoin> getCoursesByProfessorId(String professorId) {
        return courseJoinRepository.findByCourseProfessorId(professorId);
    }
    ...
    public void joinCourse(JoinCourseRequest req) {
        ...
        if (!courseJoinRepository.existsById(courseId)) {
     ...
     */
    @Transactional(readOnly = true)
    public List<CourseJoin> getEnrolledCourses(String studentIdStr) {
        // Parse student ID
        ...
        return courseJoinRepository.findAllById(courseIds);
    }
}
